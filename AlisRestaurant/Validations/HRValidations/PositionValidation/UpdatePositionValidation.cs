...
{
    public class UpdatePositionValidation : AbstractValidator<UpdatePositionRequest>
    {
        private readonly AppDbContext _context;

        public UpdatePositionValidation(AppDbContext context)
        {
            _context = context;
            ...
            RuleFor(x => x)
                .MustAsync((request, cancellationToken) => BeUniqueInDepartment(request.Id, request.DepartmentId, request.Name, cancellationToken))
                .WithMessage("Bu department-də bu adda position artıq mövcuddur")
                .When(x => !string.IsNullOrWhiteSpace(x.Name) && x.DepartmentId > 0 && x.Id > 0);

            RuleFor(x => x.DepartmentId)
                .GreaterThan(0)
                .WithMessage("Department ID mütləq daxil edilməlidir və pozitiv tam ədəd olmalıdır")
                .MustAsync(ExistDepartmentInDatabase)
                .WithMessage("Bu ID ilə department mövcud deyil");
        }

        private async Task<bool> ExistInDatabase(int positionId, CancellationToken cancellationToken)
        {
            return await _context.Positions
                .AnyAsync(p => p.Id == positionId, cancellationToken);
        }

        private async Task<bool> ExistDepartmentInDatabase(int departmentId, CancellationToken cancellationToken)
        {
            return await _context.Departments
                .AnyAsync(d => d.Id == departmentId, cancellationToken);
        }

        private async Task<bool> BeUniqueInDepartment(int positionId, int departmentId, string name, CancellationToken cancellationToken)
        {
            if (string.IsNullOrWhiteSpace(name))
                return true;

            // Eyni position-un özünü istisna edirik (adını dəyişdirmək istəyirsə)
            return !await _context.Positions
